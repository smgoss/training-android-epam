...
public class TouchListView extends ListView {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mDragListener != null || mDropListener != null) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				currentItem = (View) getChildAt(itemnum
						- getFirstVisiblePosition());

				if (isDraggableRow(currentItem)) {
					mDragPoint = y - currentItem.getTop();
					mCoordOffset = ((int) ev.getRawY()) - y;
					View dragger = currentItem.findViewById(grabberId);
					Rect r = mTempRect;
					...
					if ((r.left < x) && (x < r.right)) {
						currentItem.setDrawingCacheEnabled(true);
						// Create a copy of the drawing cache so that it does
						// not get recycled
						// by the framework when the list tries to clean up
						// memory
						Bitmap bitmap = Bitmap.createBitmap(currentItem
								.getDrawingCache());
						currentItem.setDrawingCacheEnabled(false);
	...
	private void stopDragging() {
		if (mDragView != null) {
			...
			((ImageView) mDragView).setImageDrawable(null);
			mDragView = null;
		}
		if (mDragBitmap != null) {
			mDragBitmap.recycle();
